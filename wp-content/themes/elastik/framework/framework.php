...
function azexo_admin_notices() {
    if (version_compare(phpversion(), '5.3.6', '<')) {
        ?>
        <div class="error">
            <p><?php esc_html_e('PHP version must be >= 5.3.6', 'elastik'); ?></p>
        </div>
        <?php
    }
}
...
function azexo_after_setup_theme() {
    load_theme_textdomain('elastik', get_template_directory() . '/languages');
...
function azexo_admin_scripts() {
    ...
    wp_localize_script('azexo-admin', 'azexo', array(
        ...
        'templates_configuration' => esc_html__('Templates configuration', 'elastik'),
        'fields_configuration' => esc_html__('Fields configuration', 'elastik'),
        'post_types_settings' => esc_html__('Post types settings', 'elastik'),
        'woocommerce_templates_configuration' => esc_html__('WooCommerce templates configuration', 'elastik'),
        'section_alert' => esc_html__('Please click "Ok" to confirm this info. Theme not provide CSS styles for all possible configurations in this section. Please use configurations only provided with demo content. Of course you can use it with complete freedom, but in this case it may be necessary to add CSS styles. You can save confirmation of this alert in "AZEXO Options > General settings".', 'elastik'),
...
function azexo_get_templates() {
    ...
    $azexo_templates = array_merge($azexo_templates, array(
        'post' => esc_html__('Post', 'elastik'), //default template
        'masonry_post' => esc_html__('Masonry post', 'elastik'), //fixed selector in JS
        'navigation_post' => esc_html__('Navigation post', 'elastik'), //fixed in full post navigation
        'related_post' => esc_html__('Related post', 'elastik'), //fixed in YARP template
    ));
...
function azexo_adminbarmenu($wp_admin_bar) {
    $args = array(
        'id' => 'edit-links',
        'title' => esc_html__('Edit links', 'elastik'),
        'href' => '#',
        'meta' => array(
            'class' => 'active',
        ),
...
function azexo_load_default_skin_options() {
    ...
    $options = get_option(AZEXO_FRAMEWORK);
    if (!isset($options['skin']) || empty($options['skin']) || !file_exists(get_template_directory() . '/options/' . $options['skin'] . '.json')) {
        $skin = get_template();
        $file = get_template_directory() . '/options/' . $skin . '.json';
...
function azexo_dynamic_css() {
    ...
    $options = get_option(AZEXO_FRAMEWORK);
    if (isset($options['disable_credits']) && $options['disable_credits']) {
        $css .= '.azexo-theme-credits { display: none }';
    }
    ...
    protected function comment($comment, $depth, $args) {
        ...
        <?php if ('div' != $args['style']) : ?>
            ...
            <div class="comment-data">
                <?php printf(wp_kses(__('<cite class="fn">%s</cite>', 'elastik'), array('cite' => array('class' => array()))), get_comment_author_link()); ?>
                <?php if ('0' == $comment->comment_approved) : ?>
                    <em class="comment-awaiting-moderation"><?php esc_html_e('Your comment is awaiting moderation.', 'elastik') ?></em>
                    <br />
                <?php endif; ?>
                <div class="comment-meta commentmetadata"><a href="<?php echo esc_url(get_comment_link($comment->comment_ID, $args)); ?>">
                        <?php
                        /* translators: 1: date, 2: time */
                        printf(esc_html__('%1$s at %2$s', 'elastik'), get_comment_date(), get_comment_time());
                        ?></a><?php edit_comment_link(esc_html__('(Edit)', 'elastik'), '&nbsp;&nbsp;', '');
                        ?>
                </div>
                <?php comment_text(get_comment_id(), array_merge($args, array('add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
                <div class="reply">
                    <?php comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'elastik'), 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
                </div>
...
if (function_exists('register_nav_menus')) {
    register_nav_menus(array(
        'primary' => esc_html__('Top primary menu', 'elastik'),
        'secondary' => esc_html__('Secondary menu', 'elastik'),
    ));
}
...
function azexo_widgets_init() {
    if (function_exists('register_sidebar')) {
        register_sidebar(array('name' => esc_html__('Right/Left sidebar', 'elastik'), 'id' => "sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Header sidebar', 'elastik'), 'id' => "header_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Middle sidebar', 'elastik'), 'id' => "middle_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Footer sidebar', 'elastik'), 'id' => "footer_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Dashboard sidebar', 'elastik'), 'id' => "dashboard_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Additional sidebar', 'elastik'), 'id' => "additional_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
    }
}
                    ...
                    case 'logo':
                        ...
                        <a class="site-title" href="<?php print esc_url(home_url('/')); ?>" rel="home">
                            <?php if (isset($options['logo']['url']) && !empty($options['logo']['url'])): ?>
                                <img src="<?php print esc_url($options['logo']['url']); ?>" alt="<?php esc_html_e('logo', 'elastik'); ?>">
                                <?php
                            elseif (has_custom_logo()):
                                ...
                                ?>
                                <img src="<?php print esc_url($image_url); ?>" alt="<?php esc_html_e('logo', 'elastik'); ?>">
                            <?php else: ?>
...
function azexo_pre_get_posts($query) {
    if ($query->is_main_query()) {
        ...
        $options = get_option(AZEXO_FRAMEWORK);
        $orderby_value = isset($_GET['orderby']) ? $_GET['orderby'] : 'menu_order';
...
function azexo_before_list($post_type) {
    ...
    if (isset($options[$post_type . '_custom_sorting_numeric_meta_keys']) && is_array($options[$post_type . '_custom_sorting_numeric_meta_keys'])) {
        foreach ($options[$post_type . '_custom_sorting_numeric_meta_keys'] as $meta_key) {
            if (!empty($meta_key)) {
                $orderby_options[esc_attr($meta_key) . '-desc'] = sprintf(esc_attr__('Sort by %s: high to low', 'elastik'), esc_attr($meta_key));
                $orderby_options[esc_attr($meta_key) . '-asc'] = sprintf(esc_attr__('Sort by %s: low to high', 'elastik'), esc_attr($meta_key));
            }
        }
    }
                ...
                switch ($part) {
                    case 'result_count':
                        ...
                        if ($total <= $per_page || -1 === $per_page) {
                            printf(_n('Showing the single result', 'Showing all %d results', $total, 'elastik'), $total);
                        } else {
                            printf(_nx('Showing the single result', 'Showing %1$d&ndash;%2$d of %3$d results', $total, '%1$d = first, %2$d = last, %3$d = total', 'elastik'), $first, $last, $total);
                        }
                        ?></p><?php
                        break;
                    case 'ordering':
...
function azexo_paging_nav() {
    ...
    $links = paginate_links(array(
        ...
        'prev_text' => '<i class="prev"></i>' . '<span>' . esc_html__('Previous', 'elastik') . '</span>',
        'next_text' => '<span>' . esc_html__('Next', 'elastik') . '</span>' . '<i class="next"></i>',
    ));
...
function azexo_get_the_category_list($separator = '', $parents = '', $post_id = false) {
    ...
    if (empty($categories)) {
        /** This filter is documented in wp-includes/category-template.php */
        return apply_filters('the_category', esc_html__('Uncategorized', 'elastik'), $separator, $parents);
    }
    ...
    <?php if (!is_single()): ?><a href="<?php esc_url(the_permalink()); ?>"><?php endif; ?>
        <?php if ($lazy): ?>
            <?php if ($thumbnail_size == 'full'): ?>
                <img class="image lazy <?php print esc_attr($zoom); ?>" data-src="<?php print esc_url($url[0]); ?>" alt="<?php esc_html_e('image', 'elastik'); ?>">
            <?php else: ?>
        ...
        <?php else: ?>
            <?php if ($thumbnail_size == 'full'): ?>
                <img class="image <?php print esc_attr($zoom); ?>" src="<?php print esc_url($url[0]); ?>" alt="<?php esc_html_e('image', 'elastik'); ?>">
            <?php else: ?>
...
function azexo_entry_field($name) {
    ...
    $image = (isset($options[$name . '_image']) && !empty($options[$name . '_image']['url'])) ? '<img src="' . esc_html($options[$name . '_image']['url']) . '" alt="' . esc_html__('image', 'elastik') . '">' : '';
    ...
    switch ($name) {
        ...
        case 'post_sticky':
            if (is_sticky() && is_home() && !is_paged())
                return '<span class="featured-post">' . esc_html__('Sticky', 'elastik') . '</span>';
            break;
        ...
        case 'post_comments_count':
            if (comments_open()) {
                ...
                $comments = '<a href="' . esc_url(get_comments_link()) . '"><span class="count">' . $comment_count['total_comments'] . '</span><span class="label">' . ($comment_count['total_comments'] == 1 ? esc_html__('comment', 'elastik') : esc_html__('comments', 'elastik')) . '</span></a>';
                return '<span class="comments">' . $comments . '</span>';
            }
            break;
        case 'post_read_more':
            $more_link_text = sprintf(esc_html__('Read more', 'elastik'));
...
function azexo_entry_share() {
    ...
    print '<a class="twitter-share" target="_blank" href="https://twitter.com/home?status=' . rawurlencode(esc_attr__('Check out this article: ', 'elastik')) . rawurlencode(get_the_title()) . '%20-%20' . rawurlencode(esc_url(apply_filters('the_permalink', get_permalink()))) . '"><span class="share-box"><i class="fa fa-twitter"></i></span></a>';
    if (!empty($image)) {
...
function azexo_entry_date($echo = true, $post = null) {
    if (has_post_format(array('chat', 'status'), $post))
        $format_prefix = _x('%1$s on %2$s', '1: post format name. 2: date', 'elastik');
    else
        $format_prefix = '%2$s';

    $options = get_option(AZEXO_FRAMEWORK);
    $date = sprintf('<span class="date">' . (isset($options['post_date_prefix']) ? esc_html($options['post_date_prefix']) : '') . '<a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a></span>', esc_url(get_permalink($post)), esc_attr(sprintf(esc_html__('Permalink to %s', 'elastik'), the_title_attribute(array('echo' => false, 'post' => $post)))), esc_attr(get_the_date('c', $post)), esc_html(sprintf($format_prefix, get_post_format_string(get_post_format($post)), get_the_date('', $post)))
    );
...
function azexo_comment_date($echo = true, $comment = null) {
    ...
    $options = get_option(AZEXO_FRAMEWORK);
    $date = sprintf('<span class="date">' . (isset($options['post_date_prefix']) ? esc_html($options['post_date_prefix']) : '') . '<a href="%1$s" title="%2$s" rel="bookmark"><time class="comment-date" datetime="%3$s">%4$s</time></a></span>', esc_url(get_comment_link($comment)), esc_attr(sprintf(esc_html__('Permalink to %s', 'elastik'), the_title_attribute(array('echo' => false, 'post' => $comment)))
...
function azexo_entry_gallery($attachment_ids, $carousel, $thumbnails, $img_size, $vertical = 0, $lazy = false, $zoom = '') {
    ...
    foreach ($attachment_ids as $attachment_id) {
        ...
        if (!empty($image_url)) {
            if ($lazy) {
                if ($img_size == 'full') {
                    $output .= '<img class="image lazy" data-src="' . esc_url($image_url) . '" data-popup="' . esc_url($full_image_url) . '" alt="' . esc_html__('image', 'elastik') . '">';
                } else {
                ...
                if ($img_size == 'full') {
                    $output .= '<img class="image" src="' . esc_url($image_url) . '" data-popup="' . esc_url($full_image_url) . '" alt="' . esc_html__('image', 'elastik') . '">';
                } else {
    ...
    if (!empty($date_to)) {
        ...
        <div class="time-left">
            ...
            <div class="time" data-time="<?php print date('Y/m/d H:i:s', $date_to); ?>">
                <div class="days"><span class="count"><?php print $days; ?></span><span class="title"><?php print (($days == 1) ? esc_html__('day', 'elastik') : esc_html__('days', 'elastik')); ?></span></div>
                <div class="hours"><span class="count"><?php print $hours; ?></span><span class="title"><?php print (($hours == 1) ? esc_html__('hour', 'elastik') : esc_html__('hrs', 'elastik')); ?></span></div>
                <div class="minutes"><span class="count"><?php print $minutes; ?></span><span class="title"><?php print esc_html__('min', 'elastik'); ?></span></div>
                <div class="seconds"><span class="count"><?php print $seconds; ?></span><span class="title"><?php print esc_html__('sec', 'elastik'); ?></span></div>
            </div>
        </div>
        <?php
    }
...
function azexo_infinite_scroll_js_options($options) {
    ...
    $options['loading']['msgText'] = '<em class="infinite-scroll-loading">' . esc_html__('Loading ...', 'elastik') . '</em>';
    $options['loading']['finishedMsg'] = '<em class="infinite-scroll-done">' . esc_html__('Done', 'elastik') . '</em>';
    return $options;
}
...
function azexo_get_dashboard_links() {
    ...
    $links = array(
        array(
            ...
            'title' => esc_html__('Log out', 'elastik'),
        )
    );
    $options = get_option(AZEXO_FRAMEWORK);
...
add_action('wp_footer', 'azexo_footer');

function azexo_footer() {
    print '<div class="azexo-theme-credits" style="text-align: center; font-size: 80%; margin: 10px 0;">© All rights reserved. Theme by <a href="http://azexo.com">AZEXO</a></div>';
}
